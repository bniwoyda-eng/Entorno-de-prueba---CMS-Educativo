...
import { useResourceComments } from "../api/educators.queries";
import { CustomAlert, getDateDiff } from "../../../utils";
import {
...
export const ResourceComments = ({ resourceId }: Props) => {
  ...
  const { mutate: createComment, isPending: isCreating } =
    useCreateCommentMutation(resourceId);
  const { mutate: deleteComment, isPending: isDeleting } =
    useDeleteCommentMutation(resourceId);
                ...
                <div className="flex items-center justify-between mb-2">
                  <div>
                    ...
                    <span className="text-xs text-main-400 ms-2">
                      {getDateDiff(comment.creation_date)}
                    </span>
                  </div>
